...
module.exports = {
  ...
  getById: async (req, res) => {
    try {
      ...
      if (!product) return res.status(404).json(response(404, 'produk tidak ditemukan'));
  ...
  create: async (req, res) => {
    try {
      ...
      return res.status(201).json(response(201, 'produk berhasil dibuat', product));
    } catch (error) {
      return res.status(500).json(response(500, 'server error', error));
    }
  },
  ...
  update: async (req, res) => {
    try {
      ...
      return res.status(200).json(response(200, 'produk berhasil diupdate', data));
    } catch (error) {
      return res.status(500).json(response(500, 'server error', error));
    }
  },
