...
module.exports = {
  ...
  checkout: async (req, res) => {
    ...
    try {
      ...
      // semua operasi db pakai { transaction: t }
      ...
      await t.commit(); // semua berhasil, simpan permanen
      ...
      return res
        .status(201)
        .json(response(201, "checkout berhasil", fullOrder));
    } catch (error) {
      await t.rollback(); // ada error, batalkan semua
  ...
  updateStatus: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json(response(200, "status order berhasil diupdate", updated));
    } catch (error) {
      return res.status(500).json(response(500, "server error", error.message));
    }
  },
