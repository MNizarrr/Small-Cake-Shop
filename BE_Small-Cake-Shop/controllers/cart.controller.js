...
module.exports = {
  ...
  addItem: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json(response(200, "produk ditambahkan ke keranjang", updateCart));
    } catch (error) {
  ...
  getCart: async (req, res) => {
    try {
      ...
      if (!cart)
        return res.status(200).json(response(200, "keranjang kosong", []));
  ...
  removeItem: async (req, res) => {
    try {
      ...
      if (!item)
        return res.status(404).json(response(404, "item tidak ditemukan"));

      await CartItem.destroy({ where: { id: req.params.id } });
      return res
        .status(200)
        .json(response(200, "item berhasil di hapus dari keranjang"));
    } catch (error) {
      return res.status(500).json(response(500, "server error", error.message));
    }
  },
