              ...
              <div key={order.id} className="bg-white rounded-2xl shadow-sm p-5">
                    ...
                    <div key={item.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-400">
                          {item.quantity} x Rp {Number(item.price_at_purchase).toLocaleString('id-ID')}
                        </p>
                      </div>
                ...
                <div className="border-t pt-3 flex items-center justify-between">
                  <div>
                    ...
                    {order.note && (
                      <p className="text-xs text-gray-400 mt-0.5">Catatan: {order.note}</p>
                    )}
                  </div>
              ...
              {Array.from({ length: pagination.totalPage }, (_, i) => i + 1).map(p => (
                <button
                  ...
                  className={`w-9 h-9 rounded-lg text-sm font-medium transition ${
                    page === p ? 'bg-pink-500 text-white' : 'border text-gray-600 hover:bg-gray-100'
                  }`}
                >
                  {p}
                </button>
